...
router.put("/" , upload , async (req , res , next) => {
    ...
    try {
        ...
        if (req.files.length > 1) {
            ...
            return next(new ApiErrors("you can not upload more than one image ..." , 403));
        } 
        ...
        if (!isSuperAdmin) {
            return next(new ApiErrors(JSON.stringify({
                english : "Sorry, you don't have permissions to update admin account ...",
        ...
        const updateAdmin = await Admin.findByIdAndUpdate({ _id : req.body.admin_id } , {
            $set : {
        ...
        if (req.body.delete_avatar) {
            ...
            // set default avatar 
            updateAdmin.avatar = updateAdmin.gender == "male" 
